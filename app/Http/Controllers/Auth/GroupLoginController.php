...
class GroupLoginController extends Controller
{
    ...
    public function login(Request $request) {
        ...
        else if(Auth::attempt(["username"=>$request->username,"password"=>$request->password, "verified_email"=>0], false)){
            Auth::logout();
            return redirect()->back()->withInput($request->only('username'))->with('error','Mohon lakukan verifikasi terlebih dahulu. Silahkan periksa inbox maupun spam email Anda.');
        }
