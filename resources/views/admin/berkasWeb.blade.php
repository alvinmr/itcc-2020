					...
					<table class="table table-striped table-bordered data" id="example">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{$b->group_name}}</td>
								<td>{{$b->institution}}</td>
								<td>{{ (!empty($b->berkasWeb)) ? $b->berkasWeb->file : ''}}</td>
								<td>{{(!empty($b->berkasWeb)) ? $b->berkasWeb->etc : ''}}</td>
								<td>{{ (!empty($b->berkasWeb)) ? date('d-m-Y H:i', strtotime($b->berkasWeb->created_at)) : '' }}</td>
								<td>
								    @if((!empty($b->berkasWeb)))
